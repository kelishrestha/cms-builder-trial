...
describe FindHeaderLinks do
  ...
  let(:rack_request_params) do
    Rack::MockRequest.env_for("/api/v1/users/#{user.uid}",
  ...
  context 'when current page is' do
    let(:base_url) { "<http://test_host/api/v1/users/#{user.uid}?" }
    ...
    context 'second page' do
      ...
      include_examples 'Render header links', 'first, last,
        previous as 1 and next as 3' do
        let(:header_link) do
          "#{base_url}page=1&per_page=1>; rel=\"first\", "\
    ...
    context 'third page' do
      ...
      include_examples 'Render header links', 'first,
        previous ,next and last page links' do
        let(:header_link) do
          "#{base_url}page=1&per_page=1>; rel=\"first\", "\
